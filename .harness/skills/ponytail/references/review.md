...
Vendored from DietrichGebert/ponytail `skills/ponytail-review` (MIT, upstream commit
`e3ba2aa`). Review diffs for unnecessary complexity. One line per finding: location,
...
performance are explicitly out of scope — route them to the Reviewer's normal
dimensions, not this pass. A single smoke test or assert-based self-check is the
ponytail minimum, not bloat; never flag it for deletion. Lists findings only, does
not apply the fixes.
