...
| 5 | Task queue present | `progress/loop/tasks.yaml` (or `--queue`) exists and is parseable; each entry has `task_id`, `description`, `gate_ok`, and a `budget` with `tokens` (round-protocol.md — unbudgeted tasks are not dispatched) |
...
| 11 | Traces dir writable (P6) | `progress/loop/traces/` exists and is writable — sub-agent execution traces land here every round; no trace = round invalid |
