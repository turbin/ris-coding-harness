...
版本：v0.2（Phase 0-1 已实施，Phase 2-5 为设计稿）
日期：2026-08-27
...
5. **门禁级别作为调用参数写入 state**：`observe-only`（只记录不变异，调试回路用）/ `l1-auto`（L1 自动、L2 需批准）/ `all-manual`（所有变异前暂停询问用户）。
6. **shell 只保留可选薄壳**：`run-loop.sh` 仅做「调起 agent 并 invoke rsi-loop」这一件事，供 cron/CI 等真正无人值守场景使用；交互场景下用户直接要求 agent 跑 N 轮即可。
7. **门禁强制力补强**：skill 自我约束弱于外部脚本，配可选 git pre-commit 钩子对 protected files 做硬校验——即使 agent 违规，提交也会被拒。
